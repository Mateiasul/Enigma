...
TM.prototype.intentHandlers = {
    ...
    "NextIntent": function (intent, session, response) {
        ...
        var question2 = "You are standing in front of the grand thing that holds a torch in its right hand. What is its name?";
        ...
        var questionp1 = "During WW2, when Hitler visited Paris, the French cut the lift cables on this monument so that Hitler would "
                             + "have to climb the steps if he wanted to reach the top ."
                             + " What is this significant monument?";
        ...
        if(city == 1)
        {
            ...
            else if (ok == 4)
            {
                if (text == "central park") {
                    response.ask(win + "If you want to play again say a new city!", idleMsg);
        ...
        if(city == 2)
        {
            if (ok == 1)
            {
                if (text == "eiffel tower" || text == "the eiffel tower")
                    response.ask("You are right!!! Next question. Paris beat Barcelona 4-0 in their 2017 Champions League home game in this stadium. What is it called?", "Are you still there?");
                else
                {  
                    triesLeft--;
                    response.ask(mistakeMsg);
            ...
            else if (ok == 2)
            {
                if (text == "stade de france" || text == "stadium of france")
                    response.ask("You are right again!!! Here's another. The ashes of Joan of Arc were scattered here and this river was the inspiration for lots of Impressionist painters, such as Henri Matisse. What is the name of this river?", "Are you still there?");
                else
                {
                    triesLeft--;
                    response.ask(mistakeMsg);
            ...
            else if (ok == 3)
            {
                ...
                {
                    response.ask("Right again !!! One more. What's the name of the biggest museum in the world, so big that it's impossible to visit the whole museum in one day?");
                }
                else
                {
                    triesLeft--;
